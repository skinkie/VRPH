...
void VRP::create_distance_matrix()
{
    ///
    ...
    int i,j,n;
    int type = this->edge_weight_type;
...
#if EJECT_DEBUG
        char msg_buffer[50];
        sprintf (msg_buffer,"After ejecting node %d\n", nodelist[i]);
        this->verify_routes(msg_buffer);
#endif
    }
...
void VRP::clean_route(int r, int heuristics)
{
    ...
    int rules= VRPH_INTRA_ROUTE_ONLY+VRPH_DOWNHILL+VRPH_BEST_ACCEPT+VRPH_SAVINGS_ONLY;
...
start_improving:
    ...
    {
#ifdef LOCAL_SEARCH_STATISTICS
        int step_ntried = 0, step_nbetter = 0, step_nbest = 0, step_moves = 0;
#endif
...
opm_search:
        ...
        while(i!=VRPH_DEPOT)
        {
            ...
            while (search_result)
            {
                ...
                search_result = OPM.search(this, i, rules, ntried, nbetter, nbest);
                step_ntried += ntried; step_nbetter += nbetter; step_nbest += nbest;
                if (nbest > 0)
                {
                    step_moves++;
                }
                search_step++;
#else
                search_result = OPM.search(this, i, rules);
#endif
                // JHR: make sure route start and end are updated
                if (search_result)
                    normalize_route_numbers();
            }
            i=j;
...
#if CLEAN_DEBUG
#ifdef LOCAL_SEARCH_STATISTICS
        printf("CLEAN::OPM tried_num=%d better_num=%d best_num=%d moves_num=%d\n", step_ntried, step_nbetter, step_nbest, step_moves);
#endif
...
#ifdef LOCAL_SEARCH_STATISTICS
                int ntried, nbetter, nbest;
                search_result = TPM.search(this, i, rules, ntried, nbetter, nbest);
                step_ntried += ntried; step_nbetter += nbetter; step_nbest += nbest;
                if (nbest>0)
                    step_moves += 1;
#else
                search_result = TPM.search(this, i, rules);
#endif
                // JHR: make sure route start and end are updated
                if (search_result)
                    normalize_route_numbers();
...
#if CLEAN_DEBUG
#ifdef LOCAL_SEARCH_STATISTICS
        printf("CLEAN::TPM tried_num=%d better_num=%d best_num=%d moves_num=%d\n", step_ntried, step_nbetter, step_nbest, step_moves);
#endif
...
#ifdef LOCAL_SEARCH_STATISTICS
                int ntried, nbetter, nbest;
                search_result = TO.search(this, i, rules, ntried, nbetter, nbest);
                step_ntried += ntried; step_nbetter += nbetter; step_nbest += nbest;
                if (nbest>0)
                    step_moves += 1;
#else
                search_result = TO.search(this, i, rules);
#endif
                // JHR: make sure route start and end are updated
                if (search_result)
                    normalize_route_numbers();
...
#if CLEAN_DEBUG
#ifdef LOCAL_SEARCH_STATISTICS
        printf("CLEAN::TO tried_num=%d better_num=%d best_num=%d moves_num=%d\n", step_ntried, step_nbetter, step_nbest, step_moves);
#endif
...
#ifdef LOCAL_SEARCH_STATISTICS
            int ntried, nbetter, nbest;
            //printf("DEBUG, SHOWING ROUTES BEFORE 3OPT");this->show_routes();
            search_result = ThreeO.search(this, r, rules, ntried, nbetter, nbest);
            //printf("DEBUG, SHOWING ROUTES AFTER 3OPT");this->show_routes();
            step_ntried += ntried; step_nbetter += nbetter; step_nbest += nbest;
            if (nbest>0)
                step_moves += 1;
#else
            search_result = ThreeO.search(this, r, rules);
#endif
            // JHR: make sure route start and end are updated
            if (search_result)
                normalize_route_numbers();
...
#if CLEAN_DEBUG
#ifdef LOCAL_SEARCH_STATISTICS
        printf("CLEAN::3O tried_num=%d better_num=%d best_num=%d moves_num=%d\n", step_ntried, step_nbetter, step_nbest, step_moves);
#endif
        printf("CLEAN::3O end_rlen=%f\n", end_rlen);
#endif
    }
...
void VRP::update_route(int j, VRPRoute *R)
{
    ...
    int* advance_array;// JHR: this is set to point to the next/pred array (code refactoring)
    ...
    if(R->start < R->end)
    {
        ...
        current=R->start;
        // JHR: Just guessing here, but sometimes the first advance was negative.
        //  so make sure this rare situation is handeled properly.
        if (this->next_array[current]<=0)
            advance_array = this->pred_array;
        else
            advance_array = this->next_array;
    }
    else
    {
